...
abstract class AbstractWidget implements WidgetInterface, iHaveChildren {
	...
	function import_uxon_object(\stdClass $source){
		...
		if ($uxon->has_property('id_space')){
			$this->set_id_space($uxon->get_property('id_space'));
			$uxon->unset_property('id_space');
		}
		if ($uxon->has_property('id')){
			$this->set_id($uxon->get_property('id'));
			$uxon->unset_property('id');
		}
		
		try {
			return $this->import_uxon_object_default(UxonObject::from_stdClass($source));
		} catch (UxonMapError $e) {
			throw new WidgetPropertyUnknownError($this, 'Unknown UXON property found for widget "' . $this->get_widget_type() . '": ' . $e->getMessage(), '6UNTXJE', $e);
		}
		return;
	}
	 ...
	 * 
	 * {@inheritDoc}
	 * @see \exface\Core\Interfaces\WidgetInterface::get_id_space()
	 */
	public function get_id_space() {
		if (is_null($this->id_space)){
			if ($this->get_parent() && $parent_id_space = $this->get_parent()->get_id_space()){
	 ...
	 * 
	 * {@inheritDoc}
	 * @see \exface\Core\Interfaces\WidgetInterface::set_id_space()
	 */
	public function set_id_space($value) {
		...
		if ($value != $id_space_old && $this->get_id_specified()){
			$this->set_id($this->get_id_specified());
		}
		return $this;
	}
	...
	public function get_value_widget_link(){
		...
		if ($this->get_value_expression() && $this->get_value_expression()->is_reference()){
			$link = $this->get_value_expression()->get_widget_link();
			$link->set_widget_id_space($this->get_id_space());
		}
		return $link;
	}
