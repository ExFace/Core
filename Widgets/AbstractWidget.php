...
abstract class AbstractWidget implements WidgetInterface
{
     ...
     * as the default height of a widget in the current facade)
    ...
    /**
     * Returns the id of this widget relative to its UI container - see `findUiContainer()`
     * 
     * If the container is a widget itself (e.g. a dialog), the returned id does not include the
     * prefix of that container. Otherwise the same widget would get different ids depending on
     * where its dialog was opened from. An explicitly specified id is always preferred.
     * 
     * @return string
     */
    public function getIdWithinUiContainer() : string
    {
        $uiContainer = $this->findUiContainer();
        
        // if were in a page, just use the normal Id 
        if (! ($uiContainer instanceof WidgetInterface)) {
            return $this->getId();
        }

        // if we have a user specified ID, always use that one (it doesnt contain the prefix regardless of the container)
        $idSpecified = $this->getIdSpecified(false);
        if ($idSpecified !== null && $idSpecified !== '') {
            return $idSpecified;
        }

        // if were in a dialog or popup etc., we need to strip the prefix of the container from the id
        return StringDataType::substringAfter(
            $this->getIdAutogenerated(), 
            $uiContainer->getIdAutogenerated() . UiPage::WIDGET_ID_SEPARATOR
        );
    }
}