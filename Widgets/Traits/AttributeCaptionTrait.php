...
{
    private $captionAutogenerated = null;
    ...
    public function getCaption() : ?string
    {
        ...
        if ($caption === null || $caption === '') {
            if ($this->isBoundToAttribute() === true) {
                if ($this->captionAutogenerated === null) {
                    $attr = $this->getAttribute();
                    if ($this->hasAggregator()) {
                        $func = $this->getAggregator()->getFunction();
                        $aggr = ' (' . ($func->getSymbolOfValue() ?? $func->getLabelOfValue()) . ')';
                    }
                    ...
                    if (/*$attr->isLabelForObject() === true && */$attr->getRelationPath()->isEmpty() === false && $this->isBoundToLabelAttribute() === true) {
                        $this->captionAutogenerated = $attr->getRelationPath()->getRelationLast()->getName() . $aggr;
                    } else {
                        $this->captionAutogenerated = $attr->getName() . $aggr;
                    }
                }
                return $this->captionAutogenerated;
            }
        }
