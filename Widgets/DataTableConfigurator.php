...
class DataTableConfigurator extends DataConfigurator
{
    ...
    protected function initSetupsTable(Tab $tab) : Tab
    {
        ...
        $uiContainer = $dataWidget->findUiContainer();
        // TODO #setup-slugs re-enable IF() once slug column is available
        $screenSlug = $uiContainer->getSlug();
        if (false && $uiContainer instanceof WidgetInterface) {
            $idSpecified = $dataWidget->getIdSpecified(false);
            if ($idSpecified !== null) {
                $widgetId = $idSpecified;
            } else {
                $uiContainerId = $uiContainer->getIdAutogenerated();
                $widgetId = StringDataType::substringAfter($dataWidget->getIdAutogenerated(), $uiContainerId . UiPage::WIDGET_ID_SEPARATOR);
            }
        } else {
            $widgetId = $dataWidget->getId();
        }
        ...
        $table = WidgetFactory::createFromUxonInParent($tab, new UxonObject([
            ...
            'filters' => [
                // TODO #setup-slugs replace PAGE with new SLUG attribute
                [
                    'attribute_alias' => 'PAGE',
                    'comparator' => ComparatorDataType::EQUALS,
                    'value' => $dataWidget->getPage()->getUid(),
                    'hidden' => true
                ], 
                [
