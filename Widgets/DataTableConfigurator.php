...
class DataTableConfigurator extends DataConfigurator
{
    ...
    public static function onCleanUp(OnCleanUpEvent $event) : void
    {
        ...
        $ds->getColumns()->addMultiple(['UID', 'NAME', 'SLUG', 'WIDGET_ID', 'OBJECT']);
        $ds->dataRead();
        ...
        // Setups whose widget could not be found - marked as orphaned when not a dry run.
        ...
        foreach ($ds->getRows() as $row) {
            ...
            // Only setups of widgets inside dialogs can be in the old format. Their autogenerated
            // widget id includes the prefix of the button that opened the dialog. Setups of widgets
            // sitting directly on a page do not have this prefix and are already in the new format.
            if ($widgetId === '' || $slug === '' || stripos($widgetId, 'button') === false) {
                $alreadyNewCnt++;
                continue;
            }

            // In the old format the slug was the alias of the page the widget lived on. If the slug
            // is not a page alias, the setup has already been converted (the slug is an action alias).
            ...
            // Finding a widget by its full autogenerated id can be very expensive and even run into
            // infinite loops if the widget no longer exists. Limit the search depth to stay safe.
            try {
            ...
            // If the widget sits directly on the page, the setup is already in the correct format.
        ...
        $event->addResultMessage('Widget setup cleanup (' . ($dryRun ? 'dry run' : 'applied') . '): read ' . $totalCnt . ' setup(s) - ' . $convertibleCnt . ($dryRun ? ' can be converted to the new format' : ' converted to the new format') . ', ' . $unresolvedCnt . ' could not be resolved, ' . $alreadyNewCnt . ' already in the new format.');
    }
