...
class DocsFacade extends AbstractHttpFacade
{
    ...
    protected function createResponse(ServerRequestInterface $request) : ResponseInterface
    {
        ...
        switch (true) {
            ...
            case ($request->getQueryParams()[self::URL_PARAM_RENDER] === self::URL_PARAM_RENDER_PRINT):
                ...
                $htmlString = $this->printCombinedPages($htmlString, $request->getUri()->__toString());
    ...
    /**
     * Render the given docs path as markdown
     * 
     * Supported path types:
     * 
     * - api/docs/exface/Core/Docs/index.md
     * - api/docs/exface/Core/Docs/UXON/UXON_prototypes.md?selector=
     * - exface/Core/Docs/index.md
     * - https://.../api/docs/exface/Core/Docs/index.md
     *
     * 
     * @param string $path
     * @return string
    ...
    {
        // Normalize path - remove `api/docs/` it this prefix is included
        ...
        // Prepare a fake HTTP request - e.g. https://.../.../api/docs/exface/Core/Docs/index.md
        // We need the request URL to be the same as in the case of regular navigation in the docs browser.
        // BUT we need raw markdown here, not HTML!
        $baseUrl = $this->buildUrlToFacade(false);
        $request = new ServerRequest('GET', $baseUrl . '/' . $path, [
            'Accept' => MimeTypeDataType::MARKDOWN,
        ]);
        
        // Set up the template request handler
        $template = new RawMarkdownTemplate($baseUrl);
        $reader = $this->getFileReader();
        $matcher = $this->getUrlMatcher();
        
        // Init the PSR-7 middleware bus
        $requestHandler = $this->getDocsHandler($baseUrl, $reader);
        $requestHandler->add(new FileRouteMiddleware($matcher, $this->getWorkbench()->filemanager()->getPathToVendorFolder(), $reader, $template));
        
        $response = $requestHandler->handle($request);
        $markdown = $response->getBody()->__toString();
    ...
    protected function getDocsHandler(string $baseUrl, FileReaderInterface $reader) : HttpMiddlewareBusInterface
    {
        ...
        $handler->add(new UxonPrototypePrinterMiddleware($this, $baseUrl, 'exface/Core/Docs/UXON/UXON_prototypes.md', $reader, 'selector'));
        $handler->add(new MetaObjectPrinterMiddleware($this, $baseUrl, 'exface/Core/Docs/creating_metamodels/Available_metaobjects.md', $reader, 'selector'));
