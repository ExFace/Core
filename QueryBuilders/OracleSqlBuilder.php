...
class OracleSqlBuilder extends AbstractSqlBuilder
{
    ...
    public function buildSqlQuerySelect(int $buildRun = 0)
    {
        ...
        if ($this->getLimit() > 0) {
            ...
            $core_query = "
								SELECT " . $distinct . $this->buildSqlComment($this->getMainObject()->getAliasWithNamespace()) . $core_select . $select_comment . " FROM " . $core_from . $core_join . $where . $group_by . $having . $order_by;
            ...
            $query = "\n SELECT " . $distinct . $this->buildSqlComment($this->getMainObject()->getAliasWithNamespace()) . $enrichment_select . $select_comment . " FROM
				(SELECT *
					FROM
            ...
            if (($group_by && $where) || $this->isSelectDistinct()) {
                $query = "\n SELECT " . $distinct . $this->buildSqlComment($this->getMainObject()->getAliasWithNamespace()) . $enrichment_select . $select_comment . " FROM (SELECT " . $select . " FROM " . $from . $join . $where . $group_by . $having . $order_by . ") EXFCOREQ " . $enrichment_join . $order_by;
            } else {
                $query = "\n SELECT " . $distinct . $this->buildSqlComment($this->getMainObject()->getAliasWithNamespace()) . $select . $select_comment . " FROM " . $from . $join . $where . $group_by . $having . $order_by;
            }
        }
