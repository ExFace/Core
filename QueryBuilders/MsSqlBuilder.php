...
class MsSqlBuilder extends AbstractSqlBuilder
{
    ...
    /**
     * Special property for internal use only (not to set via UXON) - marks an attribute query part as
     * requiring the `STUFF...FOR XML` grouping workaround.
     */
    const DAP_SQL_STUFF_XML = 'SQL_STUFF_XML';
    ...
    protected function buildSqlGroupByExpression(QueryPartAttribute $qpart, $sql, AggregatorInterface $aggregator){
        ...
        switch ($aggrFunc) {
            ...
            // Example of result of this branch:
            // ```sql
            //  STUFF(CAST(( SELECT DISTINCT [text()] = ', ' +
            //  Category2.Name
            // ```
            case $isListAggregation && $isAggregatedViaStuffXML:
                ...
                $qpart->setDataAddressProperty(self::DAP_SQL_STUFF_XML, true);
    ...
    protected function buildSqlSelectGrouped(QueryPart $qpart, $select_from = null, $select_column = null, $select_as = null, AggregatorInterface $aggregator = null)
    {
        ...
        switch ($function_name) {
            ...
            case $this->isAggregatedViaStuffXML($qpart, $aggregator):
                ...
                if (! $qpart->getQuery()->isSubquery()) {
                    // If the aggregating via GROUP BY (i.e. not in a subquery), we neeesd to build a subquery here
    ...
    protected function buildSqlSelectSubselect(\exface\Core\CommonLogic\QueryBuilder\QueryPart $qpart, $select_from = null)
    {
        if ($qpart->hasAggregator()) {
            ...
            $useStuffXml = $this->isAggregatedViaStuffXML($qpart, $aggregator);
            $qpart->setDataAddressProperty(self::DAP_SQL_STUFF_XML, true);
        } else {
            $useStuffXml = false;
        }
        
        // Call the regular subselect builder anyway. However, if we use `STUFF...FOR XML` we must not forget to add
        // ann appropriate ending - see below.
        $subselect = parent::buildSqlSelectSubselect($qpart, $select_from);
        
        if ($useStuffXml === true) {
            // See isAggregatedViaStuffXML() for details
            // NOTE: The beginning with the STUFF is added by $subquery::buildSqlSelectGrouped() and here we just
            // get the resulting SQL of that subquery, which is incomplete because FOR XML is still missing. We
            // add it here because it should go _after_ the WHERE generated by the subquery. We do not have access
            // to the subquery instance here - we can only rely on SQL text returned by it. In particular, the
            // $qpart here is a _different_ on from the $qpart in buildSqlSelectGrouped(), that starts the STUFF
            // because it is a different query.
            // 
            ...
            //                  DISTINCT [text()] = ', ' +
            //                  Category2.Name
            //              FROM dbo.ProductToCategory AS ProductToCategory2
            //                  LEFT JOIN dbo.Category AS Category2 ON ProductToCategory2.CategoryId = Category2.Id AND Category2.ObsoletFlag != 1
            //              WHERE
            //                  ProductToCategory2.ProductId = (Product.Id))
            // ```
