        ...
        switch ($function_name) {
            ...
            case AggregatorFunctionsDataType::LIST_ALL:
                ...
                if ($select_column !== null) {
                    $sql .= " FOR XML PATH(''), TYPE) AS VARCHAR(max)), 1, {$this->escapeString($delimLength)}, '')";
                }
                break;
        }
        return $sql;
        ...
        if ($qpart->hasAggregator()) {
            ...
            switch ($function_name) {
                ...
                case AggregatorFunctionsDataType::LIST_ALL:
                    ...
                    $delim = $args[0] ?? ', ';
                    $delimLength = strlen($delim);
                    $subselect = substr($subselect, 0, -1) . " FOR XML PATH(''), TYPE) AS VARCHAR(max)), 1, {$this->escapeString($delimLength)}, ''))";
                    break;
            }
        }
        return $subselect;
