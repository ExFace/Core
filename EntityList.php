...
class EntityList extends AbstractExfaceClass implements EntityListInterface {
	 ...
	 */
	public function __construct(exface &$exface, &$parent_object){
	 ...
	 * {@inheritDoc}
	 * 
	 * In order to instantiate a list with all it's entities, we need to know, what type of entities they are (UXON will not always have
	 ...
	 */
	public function import_uxon_object(UxonObject $uxon, $factory_class_name = null){
	 ...
	 * or had been registered for this entity list via set_entity_factory_name(). If so, factory::create_from_uxon($exface, $uxon)
	 ...
	 * {@inheritDoc}
	 * @see \exface\Core\Interfaces\EntityListInterface::import_uxon_array()
	 */
	public function import_uxon_array(array $uxon, $factory_class_name = null){
		if (is_null($factory_class_name)){
			$factory_class_name = $this->get_entity_factory_name();
		}
		if (is_null($factory_class_name)){
	...
	public function remove_all(){
		foreach ($this->get_all() as $key => $entity){
			$this->remove_by_key($key);
		}
	...
	public function get_first(){
		return array_values($this->get_all())[0];
	}
	...
	public function get_last(){
		$values = array_values($this->get_all());
	...
	public function get_nth($number){
		$i = 1;
		foreach ($this->get_all() as $entity){
			if ($i == $number){
				return $entity;
			}
		}
	 ...
	 * @return NameResolver
	 */
	public function get_entity_name_resolver() {
	 ...
	 * and factories. The name resolver takes care of resolving all the name conventions in ExFace.
	 * 
	 * @param NameResolver $value
	 */
	public function set_entity_name_resolver(NameResolverInterface $value) {
	...
	public function get_entity_factory_name() {
		if (is_null($this->endity_factory_name)){
			if ($this->get_entity_name_resolver() && $factory = $this->get_entity_name_resolver()->get_factory_class_name()){
				return $factory;
			}
		}
	...
	public function set_entity_factory_name($value) {
	 ...
	 * you want to remove all entities, use remove_all() instead. reset() is only ment for internal purposes!
	 */
