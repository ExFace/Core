...
class Progressbar extends Formula {
	...
	function run($value, $text='', $min=0, $max=100, $color_map=array()){
		if (!$value) return '';
		if (!$text) $text = $value;
		$value = NumberDataType::parse($value);
		// TODO make the color map work (currently just a demo for value = 90)
		$return = '<div style="width:100%;border:1px solid #ccc;position:relative;">' .
    			'<div style="width:' . ($value ? $value / ($max-$min) * 100 : $min) . '%;background:#' . ($value >= 90 ? 'ddd' : 'BFD297') . ';">&nbsp;</div>' .
