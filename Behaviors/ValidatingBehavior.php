...
{    
    ...
    {
        $this->getWorkbench()->eventManager()->addListener(OnBeforeCreateDataEvent::getEventName(), [$this, 'handleOnSave'], $this->getPriority());
        $this->getWorkbench()->eventManager()->addListener(OnBeforeUpdateDataEvent::getEventName(), [$this, 'handleOnSave'], $this->getPriority());
    ...
    {
        $this->getWorkbench()->eventManager()->removeListener(OnBeforeCreateDataEvent::getEventName(), [$this, 'handleOnSave']);
        $this->getWorkbench()->eventManager()->removeListener(OnBeforeUpdateDataEvent::getEventName(), [$this, 'handleOnSave']);
     ...
     */
    public function handleOnSave(DataSheetEventInterface $event)
    {
        if ($this->isDisabled()) {
            return;
        }
        ...
        /* TODO aka 23.08.2024: not sure, if we need that. Try without it first
     ...
     */
    protected function setInvalidIf(UxonObject $arrayOfDataChecksUxon) : ValidatingBehavior
    {
        $this->invalidIfUxon = $arrayOfDataChecksUxon;
    ...
    protected function getDataChecks() : DataCheckListInterface
    {
        if ($this->dataCheckList === null) {
            ...
            foreach ($this->invalidIfUxon as $uxon) {
