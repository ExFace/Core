...
class StateMachineBehavior extends AbstractBehavior
{
    ...
    protected function overrideAttributeDataType() : StateMachineBehavior
    {
        ...
        $attr->setCustomDataTypeUxon(new UxonObject(['values' => $enumVals]));
    ...
    protected function overrideAttributeEditorWidget() : StateMachineBehavior
    {
        ...
        /*
        $this->getWorkbench()->eventManager()->addListener(OnBeforePrefillEvent::getEventName(), function(OnBeforePrefillEvent $event) {
            if ($event->getWidget() instanceof iShowSingleAttribute) {
                $widget = $event->getWidget();
                if ($widget->getAttribute()->isExactly($this->getStateAttribute())) {
                    $widget->setValue($this->getEditorUxon());
