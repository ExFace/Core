...
class Object implements ExfaceClassInterface, AliasInterface {
	...
	function __construct(\exface\Core\Model\Model $model){
		$exface = $model->exface();
		$this->model = $model;
	...
	function get_relation($alias, $foreign_key_alias = ''){
		...
		if (!is_array($rel)){
			...
			$first_rel = false;
			/* @var $r \exface\Core\Model\relation */
			foreach ($rel as $r){
	 ...
	 * @return \exface\Core\AbstractDataConnector
	 */
