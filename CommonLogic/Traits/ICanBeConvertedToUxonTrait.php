...
/**
 * Contains methods to quickly implement the ICanBeConvertedToUxon interface.
 * 
 * In contrast to ImportUxonObjectTrait this trait also includes an exportUxonObject() implementation. With time,
 * we should probably replace the ImportUxonObjectTrait completely with this newer one.
 * 
 * @see exface\Core\CommonLogic\Traits\ImportUxonObjectTrait
 * @see \exface\Core\Interfaces\iCanBeConvertedToUxon
 * 
 * @author Andrej Kabachnik
 */
trait ICanBeConvertedToUxonTrait {

    private ?UxonObject $importedUxon = null;
    ...
    {
        if ($this->importedUxon === null) {
            $this->importedUxon = $uxon->copy();
        } else {
            $this->importedUxon = $this->importedUxon->extend($uxon);
        }
    ...
    /**
     * Returns the imported UXON
     * 
     * If importUxonObject() is called multiple times, the result here will be the merged UXON.
     * 
     * @see \exface\Core\Interfaces\iCanBeConvertedToUxon::getUxonSchemaClass()
     */
    public function exportUxonObject()
    {
        return $this->importedUxon;
    }
}