...
abstract class AbstractAction implements ActionInterface
{
    ...
    final public function getResultMessage()
    {
        ...
        if ($this->getResultMessageText()) {
            $message = '';
            $placeholders = $this->getWorkbench()
                ->utils()
                ->findPlaceholdersInString($this->getResultMessageText());
    ...
    {
        if ($object = $this->getWorkbench()
            ->model()
            ->getObject($qualified_alias)) {
    ...
    public function exportUxonObject()
    {
        ...
        if ($this->getCalledByWidget()) {
            $uxon->called_by_widget = $this->getCalledByWidget()
                ->createWidgetLink()
                ->exportUxonObject();
        }
    ...
    protected function dispatchEvent($event_name)
    {
        ...
        $this->getApp()
            ->getWorkbench()
            ->eventManager()
            ->dispatch(EventFactory::createActionEvent($this, $event_name));
    }
    ...
    {
        return $this->getCalledByWidget() ? $this->getCalledByWidget()->getPage() : $this->getWorkbench()
            ->ui()
            ->getPageCurrent();
    }
    ...
    public function getTemplate()
    {
        return $this->getWorkbench()
            ->ui()
            ->getTemplate($this->getTemplateAlias());
    }
    ...
    public function getTemplateAlias()
    {
        if (is_null($this->template_alias)) {
            $this->template_alias = $this->exface->ui()
    ...
    public function getTransaction()
    {
        if (is_null($this->transaction)) {
            $this->transaction = $this->getWorkbench()
                ->data()
                ->startTransaction();
        }
