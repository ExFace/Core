...
{    
    ...
    protected function findViolationsViaFilter(DataSheetInterface $data, ConditionGroupInterface $filter, ?LogBookInterface $logBook = null) : array
    {
        ...
        try {
            ...
            $logBook?->addLine('**ERROR** filtering data to check via `' . $filter->__toString() . '`. Assuming check does not apply: **' . $e->getMessage() . '**');
            $this->getWorkbench()->getLogger()->logException(new DataCheckRuntimeError($data, 'Cannot perform data check via `' . $filter->__toString() . '`. ' . $e->getMessage(), null, $e, $this));
            return [];
        }
    }
