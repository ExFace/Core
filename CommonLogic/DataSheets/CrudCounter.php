...
class CrudCounter implements WorkbenchDependantInterface
{
    ...
    public function __construct(WorkbenchInterface $workbench, int $maximumDepth = -1)
    {
        ...
        $this->eventToCounter = [
            OnCreateDataEvent::getEventName() => self::COUNT_CREATES,
            OnReadDataEvent::getEventName() => self::COUNT_READS,
            OnUpdateDataEvent::getEventName() => self::COUNT_UPDATES,
            OnDeleteDataEvent::getEventName() => self::COUNT_DELETES
        ];
    }
     ...
     *     FALSE. 
     * @param array $operationsToSkip
     * Operations contained in this array will be ignored, when counting.
     * @return $this
     * 
     * @see CrudCounter::stop()
     * @see CrudPerformedEventInterface
     */
    public function start(array $objects, bool $reset = true, array $operationsToSkip = []) : CrudCounter
    {
        if($reset) {
            $this->reset();
        }
        ...
        $this->setOperationsToSkip(array_merge($this->operationsToSkip, $operationsToSkip));
    ...
    /**
     * Stops the counter and resets all tracked variables to their default (usually `null`).
     *
     * @return void
     */
    ...
    protected function finalizeOperation(MetaObjectInterface $object, DataSheetEventInterface $event) : bool
    {
        ...
        $counter = $this->eventToCounter[$event::getEventName()];
        $skipOperation = in_array($counter, $this->operationsToSkip);

        return $inRange && !$skipOperation;
    }
     ...
     *
     * **Example**: You set the depth at `2`.
     * Then, a CREATE operation causes an UPDATE, which in turn requires a READ. The counter would register +1 CREATE
     * and
     * +1 UPDATE, ignoring the READ, because it is 3 levels deep.
     *
     * @param int $value
    ...
    /**
     * Define which operations should be skipped when counting.
     *
     * @param array $operationsToSkip
     * @return $this
     */
    public function setOperationsToSkip(array $operationsToSkip) : CrudCounter
    {
        if(in_array(self::COUNT_WRITES, $operationsToSkip)) {
            $operationsToSkip[] = self::COUNT_CREATES;
            $operationsToSkip[] = self::COUNT_UPDATES;
        }

        $this->operationsToSkip = array_unique($operationsToSkip);
     ...
     * @return array
     */
    public function getOperationsToSkip() : array
    {
        return $this->operationsToSkip;
    }
}