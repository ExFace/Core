...
class XLSXWriter
{
	...
	protected function initializeSheet($sheet_name, $col_options=array(), $auto_filter=false, $freeze_rows=false, $freeze_columns=false )
	{
		...
		if (! empty($col_options)) {
			foreach($col_options as $opt) {
			    if (! is_array($opt)) {
			        // Backward compatibility to $col_width array (upto version 0.37)
			        $width = 'customWidth="true" width="'.floatval($opt).'"';
			    } elseif ($opt['width'] !== null) {
                    $width = 'customWidth="true" width="'.floatval($opt['width']).'"';
			    } else {
			        $width = 'customWidth="false" width="11.5"';
			    }
			    $hidden = $opt['hidden'] !== true ? 'false' : 'true';
			    $collapsed = $opt['collapsed'] !== true ? 'false' : 'true';
				$sheet->file_writer->write(  '<col collapsed="'.$collapsed.'" hidden="'.$hidden.'" max="'.($i+1).'" min="'.($i+1).'" style="0" '.$width.'/>');
				$i++;
			}
		}
	...
	public function writeSheetHeader($sheet_name, array $header_types, $options = null, array $columns = [])
	{
		if (empty($sheet_name) || empty($header_types) || !empty($this->sheets[$sheet_name]))
			return;

		$suppress_row = isset($options['suppress_row']) ? intval($options['suppress_row']) : false;
		if (is_bool($options))
		{
			...
			$suppress_row = intval($options);
		}
        $style = &$options;
        
        // Fallback to support the definition of an array of widths in the $options parameter (used upto version 0.37)
        if ($options['widths'] !== null) {
            $col_widths = (array)$options['widths'];
            foreach ($col_widths as $nr => $width) {
               $columns[$nr]['width'] = $width;
            }
        }

		$auto_filter = isset($options['auto_filter']) ? intval($options['auto_filter']) : false;
		$freeze_rows = isset($options['freeze_rows']) ? intval($options['freeze_rows']) : false;
		$freeze_columns = isset($options['freeze_columns']) ? intval($options['freeze_columns']) : false;
		self::initializeSheet($sheet_name, $columns, $auto_filter, $freeze_rows, $freeze_columns);
		$sheet = &$this->sheets[$sheet_name];
