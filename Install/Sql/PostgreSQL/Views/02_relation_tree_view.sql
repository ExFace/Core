...
CREATE OR REPLACE VIEW exf_relation_tree AS
(
SELECT 
    rt.*,
...
FROM (
	SELECT 
		CONCAT(REPLACE(a.oid::text, '-', ''), REPLACE(a.object_oid::text, '-', '')) AS oid,
	...
	SELECT 
        CONCAT(REPLACE(ar.oid::text, '-', ''), REPLACE(ar.related_object_oid::text, '-', '')) AS oid,
