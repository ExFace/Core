...
class OracleSqlConnector extends AbstractSqlConnector
{
    ...
    protected function performQuerySql(SqlDataQuery $query)
    {
        ...
        // FIXME how to properly define which column contains the insert id???
        ...
        if (! $result) {
            throw new DataQueryFailedError($query, "Pasrsing of a query to the database failed!" . ($e ? ' ' . $e->getMessage() : ''), $this->getErrorCode($e), $e);
        } else {
            ...
            if (! $ex) {
                throw new DataQueryFailedError($query, ($this->getLastError() ? $this->getLastError() : ($e ? ' ' . $e->getMessage() : '')), '6T2T2UI', $e);
                return $query;
            } else {
    ...
    protected function setInsertIdFieldName($value)
    {
    ...
    /**
     * The oracle SID to be used in this connection
     *
     * @uxon-property sid
     * @uxon-type string
     *
