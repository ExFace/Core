...
class ShowDialog extends ShowWidget implements iShowDialog
{
     ...
     * 
     * For example, consider the following dialog widget:
     * 
     * ```
     *  {
     *      "widget_type": "Dialog",
    ...
    protected function addIdSpaceToWidgetUxon(UxonObject $uxon) : UxonObject
    {
        ...
        // Otherwise the id space would be the id of the parent. However, we prefer the autogenerated id because
        // it is a path and is found much faster inside a page then a user-provided id.
        $parentId = $parent->getIdAutogenerated();
        if ($parentId !== null) {
            // If we have an autogenerated id - use that. It does not need a namespace because it's a path
            // starting from the page root.
            $idSpace = $parentId;
        } else {
            // If we don't have an autogenerated id, use the id explicitly set by a user (or sometimes also by
            // some programming logic). In this case we will also need an id space. The id of the parent widget
            // can actually include the id space already or just be a custom string. If it already includes its
            // id space, we can use it as-is, but if not - we need to prepend the id space of the parent. In the
            // end we will have an id like this: `space.parent.ours` for our dialog.
