...
class ShowDataImportDialog extends ShowDialog
{
    ...
    protected function createDialogWidget(UiPageInterface $page, WidgetInterface $contained_widget = NULL)
    {
        ...
        foreach ($colAttrs as $i => $attr) {
            ...
            if ($attr->isRelation()) {
                // Generated by GitHub Copilot - Sep-02-2026
                // Honor the attribute's default editor (so a curated value/text config is kept) instead of
                // overwriting it with a bare combo, which would fall back to the relation right key and may
                // yield a label instead of the key. Only force non-lazy loading for the import grid.
                $editorUxon = $attr->getDefaultEditorUxon()->copy();
                if (! $editorUxon->hasProperty('widget_type')) {
                    $editorUxon->setProperty('widget_type', 'InputComboTable');
                }
                $editorUxon->setProperty('lazy_loading', false);
                $colUxon->setProperty('cell_widget', $editorUxon);
            }
